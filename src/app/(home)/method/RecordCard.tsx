        ...
        {methods.map((item, index) => (
          <Link key={index} href={`/method/${item.id}`}>
            <div className='bg-darkGray rounded-lg px-5 py-3 w-full h-[150px]'>
              <p className='text-xl font-bold text-lightGray'>{ item.name }</p>
              <p className='font-bold line-clamp-3'>{ item.detail }</p>
              <p className='font-bold text-lightGray line-clamp-1'>{ item.memo }</p>
            </div>
          </Link>
        ))}
      </div>
